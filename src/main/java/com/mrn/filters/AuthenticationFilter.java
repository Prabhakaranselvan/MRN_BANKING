...
public class AuthenticationFilter implements Filter
{
			...
			throws IOException, ServletException
	{
		...
		// Allow unauthenticated access to GET+GET /branch for signup page
		if ("/branch".equals(path) && "GET".equalsIgnoreCase(req.getMethod()) && "GET".equalsIgnoreCase(headerMethod))
		{
			System.out.println("[AuthFilter] Allowing public access to GET /branch");
		...
		if (EXCLUDED_PATHS.matcher(path).matches())
		{
			...
			{
				writeJsonError(res, HttpServletResponse.SC_BAD_REQUEST,
						"Clear (or) logout the existing session to proceed.");
			}
			return;
		}
