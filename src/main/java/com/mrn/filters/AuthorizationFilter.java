...
public class AuthorizationFilter implements Filter
{
			...
			throws IOException, ServletException
	{
		...
		if ("/branch".equals(path) && "GET".equalsIgnoreCase(headerMethod) && "GET".equalsIgnoreCase(method))
		{
			System.out.println("[AuthorizationFilter] Public GET /branch access allowed (no session).");
		...
		System.out.println("[AuthzFilter] Checking access for path: " + path + ", headerMethod: " + headerMethod
				+ ", method: " + method + ", role: " + userRole);
		...
		if (!authorized)
		{
			writeJsonError(res, HttpServletResponse.SC_FORBIDDEN,
					"Access denied: Your role (" + userRole + ") is not permitted to access " + path);
			return;
		}
